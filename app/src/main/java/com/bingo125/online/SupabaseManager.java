...
public final class SupabaseManager {
    ...
    private static boolean[][] marks(int[][] card,List<Integer> called){boolean[][] g=new boolean[5][5];if(card==null)return g;for(int r=0;r<5;r++)for(int c=0;c<5;c++)g[r][c]=called.contains(card[r][c]);return g;}
    private static List<String> completed(int[][] card,List<Integer> called){List<String> out=new ArrayList<>();if(card==null)return out;boolean[][] m=marks(card,called);for(int r=0;r<5;r++){boolean ok=true;for(int c=0;c<5;c++)ok&=m[r][c];if(ok)out.add("R"+(r+1));}for(int c=0;c<5;c++){boolean ok=true;for(int r=0;r<5;r++)ok&=m[r][c];if(ok)out.add("C"+(c+1));}boolean a=true,d=true;for(int i=0;i<5;i++){a&=m[i][i];d&=m[i][4-i];}if(a)out.add("D1");if(d)out.add("D2");return out;}
}
