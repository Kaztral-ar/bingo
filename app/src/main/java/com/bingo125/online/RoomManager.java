...
public class RoomManager {
    ...
    private void reportError(String prefix,String message){if(listener!=null)main.post(()->listener.onError(prefix+message));}
    ...
    public void requestNextNumber(String code,String uid){callNumber(code,new SupabaseManager.Callback(){public void success(JSONObject r){}public void error(String m){reportError("Calling number failed: ",m);}});}
    public void callNumber(String code,SupabaseManager.Callback cb){try{JSONObject b=new JSONObject();b.put("action","call");b.put("roomCode",code);send(b,cb);}catch(Exception e){if(cb!=null)cb.error(e.getMessage());else reportError("Calling number failed: ",e.getMessage());}}
