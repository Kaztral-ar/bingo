...
public class RoomManager {
    ...
    // Card writes are serialized so an older HTTP request can never overwrite a newer card.
    private final Object cardLock=new Object();
    private final Queue<CardWrite> cardQueue=new ArrayDeque<>();
    private boolean cardWriteRunning;

    private static class CardWrite {
        final String code, uid;
        final int[][] card;
        final boolean locked;
        CardWrite(String code,String uid,int[][] card,boolean locked){this.code=code;this.uid=uid;this.card=copy(card);this.locked=locked;}
    }
    ...
    /** Enqueues a snapshot of the card; writes are sent strictly in tap order. */
    public void saveCard(String code,String uid,int[][] card,boolean locked){
        if(context==null||card==null)return;
        synchronized(cardLock){cardQueue.add(new CardWrite(code,uid,card,locked));if(!cardWriteRunning){cardWriteRunning=true;writeNextCard();}}
    }

    private void writeNextCard(){
        final CardWrite w;
        synchronized(cardLock){w=cardQueue.poll();if(w==null){cardWriteRunning=false;return;}}
        try{
            JSONObject b=new JSONObject();
            b.put("action","saveCard");
            b.put("roomCode",w.code);
            b.put("card",grid(w.card));
            b.put("locked",w.locked);
            send(b,new SupabaseManager.Callback(){
                public void success(JSONObject r){writeNextCard();}
                public void error(String m){
                    if(listener!=null)main.post(()->listener.onError("Card save failed: "+m));
                    writeNextCard();
                }
            });
        }catch(Exception e){
            if(listener!=null)main.post(()->listener.onError("Card save failed: "+e.getMessage()));
            writeNextCard();
        }
    }
    ...
    public void requestNextNumber(String code,String uid){callNumber(code,null);}
    ...
    private void poll(){
        if(!listening)return;
        try{
            JSONObject b=new JSONObject();b.put("action","getRoom");b.put("roomCode",activeRoom);
            send(b,new SupabaseManager.Callback(){
                public void success(JSONObject r){
                    try{RoomModel room=SupabaseManager.parseRoom(r);main.post(()->{if(listening&&listener!=null)listener.onRoomUpdated(room);});}
                    catch(Exception e){main.post(()->{if(listener!=null)listener.onError("Could not read game room.");});}
                    if(listening)main.postDelayed(()->poll(),700);
                }
                public void error(String m){if(listening)main.postDelayed(()->poll(),1200);}
            });
        }catch(Exception ignored){if(listening)main.postDelayed(()->poll(),1200);}
    }
