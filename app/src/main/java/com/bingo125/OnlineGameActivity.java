...
public class OnlineGameActivity extends AppCompatActivity {
    ...
    @Override protected void onCreate(Bundle b){
        ...
        grid=findViewById(R.id.bingoGrid);status=findViewById(R.id.textStatusLabel);big=findViewById(R.id.textBigNumber);timer=findViewById(R.id.textTimer);footer=findViewById(R.id.textFooterStatus);calledRow=findViewById(R.id.calledNumbersRow);calledScroll=findViewById(R.id.calledScroll);callButton=findViewById(R.id.btnCallNumber);callButton.setVisibility(View.VISIBLE);callButton.setOnClickListener(v->callNumber());buildGrid();
    ...
    private void callNumber(){if(resultShown||latest==null||!"calling".equals(latest.status))return;callButton.setEnabled(false);rooms.callNumber(roomCode,new SupabaseManager.Callback(){public void success(org.json.JSONObject r){}public void error(String m){runOnUiThread(()->{if(!resultShown)Toast.makeText(OnlineGameActivity.this,m,Toast.LENGTH_SHORT).show();});}});}
    private void showResult(RoomModel r){resultShown=true;rooms.stopListening();stopFillTimer();boolean tie="TIE".equalsIgnoreCase(r.winningPattern);boolean won=!tie&&myUid.equals(r.winnerUid);if(won)sound.playWin();new StatsManager(this).recordOnlineGame(won,lastCalls);ads.showInterstitialIfReady(this);RoomModel.PlayerModel p=r.players.get(r.winnerUid);Intent i=new Intent(this,ResultActivity.class);i.putExtra("winnerName",tie?"Both Players":(p==null?"Opponent":p.name));i.putExtra("pattern",tie?"TIE":(r.winningPattern==null?"B-I-N-G-O":r.winningPattern));i.putExtra("mode","online");startActivity(i);finish();}
