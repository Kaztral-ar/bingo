...
public class OnlineGameActivity extends AppCompatActivity {
    private final RoomManager rooms=new RoomManager(); private String roomCode,myUid; private RoomModel latest; private boolean resultShown,promptShown; private CountDownTimer fillTimer; private int[][] myCard=new int[5][5]; private int nextNumber=1; private int lastCalls; private boolean localCardDirty;
    ...
    private void buildGrid(){grid.removeAllViews();for(int r=0;r<5;r++)for(int c=0;c<5;c++){TextView v=new TextView(this);GridLayout.LayoutParams p=new GridLayout.LayoutParams();p.width=0;p.height=0;p.rowSpec=GridLayout.spec(r,1f);p.columnSpec=GridLayout.spec(c,1f);int m=(int)(getResources().getDisplayMetrics().density*3);p.setMargins(m,m,m,m);v.setLayoutParams(p);v.setGravity(Gravity.CENTER);v.setTextSize(18);v.setBackgroundColor(getColor(R.color.bg_cell));v.setTextColor(getColor(R.color.text_primary));final int rr=r,cc=c;v.setOnClickListener(x->place(rr,cc));grid.addView(v);cells[r][c]=v;}}
    private void render(RoomModel r){if(resultShown)return;latest=r;if("filling".equals(r.status)){status.setText("Fill Your Bingo Card");footer.setText("Tap cells to place 1 → 25");renderMyCard(r,false);if(r.fillDeadline!=null&&fillTimer==null){long rem=Math.max(0,r.fillDeadline-System.currentTimeMillis());fillTimer=new CountDownTimer(rem,1000){public void onTick(long x){timer.setText(String.format("Time Left: %d:%02d",x/60000,(x/1000)%60));}public void onFinish(){timer.setText("Time Left: 0:00");maybeStartCalling(r);}}.start();}if(allLocked(r))maybeStartCalling(r);}
        else if("calling".equals(r.status)){if(fillTimer!=null){fillTimer.cancel();fillTimer=null;}renderMyCard(r,true);calledScroll.setVisibility(View.VISIBLE);renderCalls(r.calledNumbers);RoomModel.PlayerModel me=r.players.get(myUid);int count=me==null?0:Math.min(5,me.bingoCount);status.setText("B-I-N-G-O  •  "+progress(count));String first=r.firstCaller==null?r.host:r.firstCaller;String other=otherPlayer(r,first);int calls=r.calledNumbers==null?0:r.calledNumbers.size();String expected=calls%2==0?first:other;boolean myTurn=myUid.equals(expected);callButton.setEnabled(myTurn);callButton.setText(myTurn?"CALL NUMBER":"WAIT FOR OPPONENT");footer.setText(myTurn?"Your turn — call one number. Both cards mark automatically.":"Opponent's turn — wait for the next call.");}
        else if("finished".equals(r.status)&&r.winnerUid!=null)showResult(r);}
    private void renderMyCard(RoomModel r,boolean forceServerCard){RoomModel.PlayerModel me=r.players.get(myUid);if(me==null||me.card==null)return;if(forceServerCard||!localCardDirty){myCard=copy(me.card);int max=0;for(int i=0;i<5;i++)for(int j=0;j<5;j++)max=Math.max(max,myCard[i][j]);if(!forceServerCard&&max>0)nextNumber=Math.min(26,max+1);if(forceServerCard)localCardDirty=false;for(int i=0;i<5;i++)for(int j=0;j<5;j++){cells[i][j].setText(String.valueOf(myCard[i][j]));boolean marked=r.calledNumbers!=null&&r.calledNumbers.contains(myCard[i][j]);cells[i][j].setBackgroundColor(marked?getColor(R.color.cell_marked):getColor(R.color.bg_cell));}}else{for(int i=0;i<5;i++)for(int j=0;j<5;j++){cells[i][j].setText(String.valueOf(myCard[i][j]));boolean marked=r.calledNumbers!=null&&r.calledNumbers.contains(myCard[i][j]);cells[i][j].setBackgroundColor(marked?getColor(R.color.cell_marked):getColor(R.color.bg_cell));}}}
