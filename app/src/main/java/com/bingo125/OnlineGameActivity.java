...
public class OnlineGameActivity extends AppCompatActivity {
    ...
    private void renderMyCard(RoomModel r){RoomModel.PlayerModel me=r.players.get(myUid);if(me==null||me.card==null)return;myCard=me.card;int max=0;for(int i=0;i<5;i++)for(int j=0;j<5;j++){max=Math.max(max,myCard[i][j]);cells[i][j].setText(String.valueOf(myCard[i][j]));boolean marked=r.calledNumbers!=null&&r.calledNumbers.contains(myCard[i][j]);cells[i][j].setBackgroundColor(marked?getColor(R.color.cell_marked):getColor(R.color.bg_cell));}if("filling".equals(r.status))nextNumber=Math.min(26,max+1);}
    private void place(int row,int col){if(latest==null||!"filling".equals(latest.status)||latest.players.get(myUid)==null||latest.players.get(myUid).cardLocked)return;if(myCard[row][col]!=0||nextNumber>25)return;myCard[row][col]=nextNumber++;cells[row][col].setText(String.valueOf(myCard[row][col]));sound.playPlace();rooms.saveCard(roomCode,myUid,myCard,nextNumber>25);if(nextNumber>25)footer.setText("Card complete — waiting for opponent…");}
