...
public class OnlineGameActivity extends AppCompatActivity {
    ...
    private void buildCallPad(){callPad.removeAllViews();for(int n=1;n<=25;n++){Button b=new Button(this);b.setText(String.valueOf(n));b.setTextSize(14);GridLayout.LayoutParams p=new GridLayout.LayoutParams();p.width=0;p.height=(int)(42*getResources().getDisplayMetrics().density);p.rowSpec=GridLayout.spec((n-1)/5,1f);p.columnSpec=GridLayout.spec((n-1)%5,1f);p.setMargins(2,2,2,2);b.setLayoutParams(p);final int number=n;b.setOnClickListener(v->callNumber(number));callPad.addView(b);callButtons[n-1]=b;}}
    ...
    private void showResult(RoomModel r){resultShown=true;rooms.stopListening();stopFillTimer();callPad.setVisibility(View.GONE);boolean tie="TIE".equalsIgnoreCase(r.winningPattern);boolean draw="DRAW".equalsIgnoreCase(r.winningPattern);boolean won=!tie&&!draw&&myUid.equals(r.winnerUid);if(won)sound.playWin();new StatsManager(this).recordOnlineGame(won,r.calledNumbers==null?0:r.calledNumbers.size());ads.showInterstitialIfReady(this);RoomModel.PlayerModel p=r.players.get(r.winnerUid);Intent i=new Intent(this,ResultActivity.class);i.putExtra("winnerName",tie?"Both Players":draw?"No Winner":(p==null?"Opponent":p.name));i.putExtra("pattern",tie?"TIE":draw?"DRAW":(r.winningPattern==null?"B-I-N-G-O":r.winningPattern));i.putExtra("mode","online");startActivity(i);finish();}
